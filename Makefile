...
run: $(ISO)
	...
	$(QEMU) -cdrom $(ISO) -serial stdio -m 512M -vga std -display cocoa,zoom-to-fit=on

# Run OS in Fullscreen QEMU (scaled to entire display)
run-fullscreen: $(ISO)
	@echo "Starting Vailism OS in Fullscreen QEMU (Ctrl+Alt+G to release)..."
	$(QEMU) -cdrom $(ISO) -serial stdio -m 512M -vga std -full-screen -display cocoa,zoom-to-fit=on
